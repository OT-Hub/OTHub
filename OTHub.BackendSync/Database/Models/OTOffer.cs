...
namespace OTHub.BackendSync.Models.Database
{
    public class OTOffer
    {
        ...
        {
            var count = connection.QueryFirstOrDefault<Int32>("SELECT COUNT(*) FROM OTOffer WHERE OfferID = @offerID", new
            {
...
@DataSetSizeInBytes, @TokenAmountPerHolder, @HoldingTimeInMinutes, @LitigationIntervalInMinutes, @IsFinalized, @FinalizedTransactionHash, @FinalizedBlockNumber, @FinalizedTimestamp, NULL)",
        ...
        {
            var count = connection.Execute("UPDATE OtOffer SET FinalizedBlockNumber = @logBlockNumber, FinalizedTransactionHash = @logTransactionHash, FinalizedTimestamp = @FinalizedTimestamp, IsFinalized = 1 WHERE OfferID = @offerId And IsFinalized = 0", new
            {
                offerId,
            ...
            {
                added = OTOfferHolder.Insert(connection, offerId, holder, true);
            }
