...
namespace OTHelperNetStandard.Tasks
{
    public class SyncHoldingContractTask : TaskRun
    {
        public override async Task Execute(Source source)
        {
            ...
            {
                foreach (var contract in OTContract.GetByType(connection, (int)ContractType.Holding))
                {
                    ...
                    var holdingContract = new Contract(eth, Constants.GetContractAbi(ContractType.Holding), contract.Address);
